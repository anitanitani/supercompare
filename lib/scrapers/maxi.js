...
const BASE_URL = 'https://www.maxiconsumo.com';

export async function scrapeMaxi(searchTerm) {
  const results = [];
  ...
  const cardSelectors = [
    ...
    'article.product', '.product-item', '.product',
  ];
  ...
  if (!results.length) {
    throw new Error('Sin resultados en Maxi Consumo — puede requerir login o los selectores cambiaron.');
  }
